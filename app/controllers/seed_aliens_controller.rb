class SeedAliensController < ApplicationController
  ...
  def create
    ...
    results_number = params[:number_of_results]
    user_input = user_input.gsub(" ","+")
    @alien = get_movie_info("http://api.themoviedb.org/3/search/movie?api_key=e753e57cc01e48ed3a8d4276010c8092&include_adult=true&query=#{user_input}")
    @alien['results'].each_with_index do |movie,index|
      if movie['poster_path'] != nil && index < results_number.to_i
